	...
	for (int i = 0; i < 6; i++)//loop runs through rows of the backend array.
	{
		for (int j = 0; j < 6; j++)//loop runs through columns of the backend array.
		{
			if ((arr[i][j]!=0) && ((arr[i][j] % 10) == (arr[i][j + 1] % 10)) && ((arr[i][j] % 10) == (arr[i][j + 2] % 10)) && ((arr[i][j] % 10) == (arr[i + 1][j] % 10)) && ((arr[i][j] % 10) == (arr[i][j + 2] % 10)))
			{
				...
				arr[i][j] = (value%10) + 20;		//Destroyer gem is made
...
void cubemaker(int board[8][8])
{
	/*This funtion takes checks if 5 consecutive gems exist anywhere
	on the board. Vertically, or horizontally. It deletes those gems
	and places a cube gem there. it also checks if any of these gems 
	is a special gem, and deals with them accordingly.*/
	int value;
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
...
{
	/*This function takes as a parameter, a gem
	from the board. It then runs through the board
	and deletes all the gems of that kind, that are
	are present anywhere on the board.*/
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
		{
...
{
	/*This function makes a square in the board
	which will be the cursor.*/
...
There are a total of 17 gems. Gem1,Gem2,Gem3,Gem4 and Gem5.
Each Gem has 2 of its own special gems. For example, Gem1
will have a gem1flaming and a gem1destroyer. There is a cube
gem also, which can destroy all the occurences of one type of
gem. There is also an empty gem, which is basically a black 
square, used to delete any gem from the board.*/
...
{
	myRhombus(k + 8, l + 8, k + 40, l + 40, 255, 255, 100, 255, 255, 100);
}

void gem3flaming(int k, int l)
{
	myRhombus(k + 8, l + 8, k + 40, l + 40, 255, 255, 255, 255, 255, 100);
}

void gem3destroyer(int k, int l)
{
	myRhombus(k + 8, l + 8, k + 40, l + 40, 135, 206, 250, 255, 255, 100);
}
...
int main()
{
	...
	int starting;
	_getch();
	//int timeleft=60-(time(0)-starttime);
	...
	//			i++;
	//		}
	//		if (key == 6 && pressed == true)	//save game
	//		{
	...
	//				for (int j = 0; j < 8; j++)
	//				{
	//					storedata << game[i][j] << endl;
	//				}
	//			}
	//			storedata << score << endl << timeleft << endl;
	//		}
	//		if (key == 7 && pressed==true)	//load game
	//		{
	...
	//			placegems(game);
	//			int loadtime;
	//			readdata >> score >> loadtime;
	//			starttime = starttime + (loadtime - timeleft);
	//			gotoxy(0, 42);
	//			cout << "	   Score:	         ";
	//		}
	...
	//		cursor(row, col, 255, 255, 255);
	//	}
	//	timeleft = 60 - (time(0) - starttime);
	//}
	//gotoxy(0, 0);
	//cout << "**********************************Game Over**********************************" << endl;
	//cout << "******************************* Happy Coding *****************************";
	//cout << "****************** Please close the window to exit the game *******************";
	//while (true);
}